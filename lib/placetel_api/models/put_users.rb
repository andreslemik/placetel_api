...
module PlacetelAPI
  ...
  class PutUsers
    ...
    # Grant rights for these numbers
    attr_accessor :rights_numbers

    # Language (ISO-639-1 Code) only de / en is supported
    attr_accessor :locale

    # Base64 encoded image as [data uri](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URIs)
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PlacetelAPI::PutUsers` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PlacetelAPI::PutUsers`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [first_name, last_name, email, did, callerid, primary_sip_user_id, rights_numbers, locale, avatar].hash
    end
